...
static int handle_pte_fault(struct vm_fault *vmf)
{
    ...
    // get the process id of the refault page
	if (!pte_present(vmf->orig_pte)){
        struct mm_struct *mpc_vm = vmf->vma->vm_mm;
        pid_t freeze_pid = mpc_vm->owner->pid;
		mpc_netlink_send (freeze_pid);
        return do_swap_page(vmf); // move the earlier evicted page back
