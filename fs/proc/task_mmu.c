				...
				size_t count, loff_t *ppos)
{
	...
	if (!strcmp(type_buf, "soft"))
		type = RECLAIM_SOFT;
    else if (!strcmp (type_buf, "freeze"))
        type = MPC_FREEZE;
    ...
    else if (type == MPC_FREEZE) {
        pid_t mpc_pid = mm->owner->pid;
        struct task_struct * mpc_t = find_task_by_vpid(mpc_pid);
        unsigned long flags;
        static DEFINE_SPINLOCK(mpc_freezer_lock);
        if (freezer_should_skip(mpc_t))
            return false;
        spin_lock_irqsave(&mpc_freezer_lock, flags);
        if (!freezing(mpc_t) || frozen(mpc_t)){
            spin_unlock_irqrestore(&mpc_freezer_lock, flags);
            return false;
        }
        if (!(mpc_t->flags & PF_KTHREAD)){
            //fake_signal_wake_up(mpc_t);
            unsigned long flags;
            if (lock_task_sighand(mpc_t, &flags)){
                signal_wake_up(mpc_t,0);
                unlock_task_sighand(mpc_t, &flags);
            }
        }
        else
            wake_up_state(mpc_t, TASK_INTERRUPTIBLE);
        spin_unlock_irqrestore(&mpc_freezer_lock, flags);
    }
